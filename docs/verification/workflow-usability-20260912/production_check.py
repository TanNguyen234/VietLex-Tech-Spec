...
with httpx.Client(cookies=cookies, timeout=120, follow_redirects=False) as client:
    paths = [
        ...
        '/readyz',
        '/',
        '/search?q=45%2F2019%2FQH14',
        '/evaluation-lab',
        state['workspace'],
        state['workspace'] + '/sources/cdb49180-70b6-447a-a233-9476b4ce1c5e',
        delivery['new_report'],
    ]
    for path in paths:
        ...
        if '/sources/' in path:
            item['saved_reader_rendered'] = 'data-saved-analysis' in r.text
        if '/reports/' in path:
            item['report_preview_rendered'] = 'report-paper' in r.text
        if path == state['workspace']:
            item['source_library_rendered'] = 'source-library-card' in r.text
        checks.append(item)
        print(path, r.status_code, flush=True)
    for path in ['app/static/css/research-workflow.css', 'app/static/js/workflow-controls.js', 'app/static/favicon.svg']:
        ...
        checks.append({
            ...
            'package_byte_match': r.content == package,
            'git_text_match_after_crlf_normalization': r.content.replace(b'\r\n', b'\n') == expected,
