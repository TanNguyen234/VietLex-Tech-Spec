...
s = get_settings()
root = Path('tmp') / ('online10-rerun-' + str(time.time_ns()))
root.mkdir(parents=True)
cases = json.loads(Path(__file__).with_name('online10-2026-cases.json').read_text(encoding='utf-8'))
...
async def main():
    for case in cases:
        ...
        try:
            remote = await asyncio.to_thread(store.search, case['reference_number'], limit=20)
        except Exception as err:
            remote = {'error': type(err).__name__}
        ...
        if out['absence_verified']:
            plan = build_research_plan(case['question'])
