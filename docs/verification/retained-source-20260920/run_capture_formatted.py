...
from app.services import retained_source_analysis as service  # noqa: E402
from app.services.clients import close_clients  # noqa: E402

root = Path('tmp/continuation-20260919/retained-ten-metadata')
...
config = {
    'use_legacy_free_pipeline': settings.USE_LEGACY_FREE_PIPELINE,
    'vertex_model': settings.VERTEX_LLM_MODEL,
    'max_output_tokens': 4096,
    'thinking_level': 'MINIMAL',
    'max_characters': service.MAX_CHARACTERS,
    'max_words': service.MAX_WORDS,
}
manifest = {
    'run_id': root.name,
    'started_at': datetime.datetime.now(datetime.timezone.utc).isoformat(),
    'git_sha': subprocess.check_output(git + ['rev-parse', 'HEAD'], text=True).strip(),
    'git_dirty': bool(subprocess.check_output(git + ['status', '--porcelain', '--untracked-files=no'])),
    ...
    'command': '.venv/Scripts/python.exe tmp/continuation-20260919/retained_ten_metadata.py',
    'dataset_revision': 'captured public official PDF OCR 20260913; original ten questions unchanged',
    'dataset_sha256': {},
    'metric_version': 'retained-source-operational-v1',
    'scope': 'real service/provider with captured real OCR; no HTTP route or legal-correctness certification',
}
for name in cases:
    for kind, path in [
        ('source', Path('tmp/data-integrity-20260913/full-reading') / name / 'result.json'),
        ('question', Path('tmp/selected-context-20260914/ten-live') / (name + '.json')),
    ]:
        manifest['dataset_sha256'][name + '-' + kind] = hashlib.sha256(path.read_bytes()).hexdigest()
for name in cases:
    for path in sorted((Path('tmp/data-integrity-20260913/full-reading') / name).glob('attempt-*.json')):
        manifest['dataset_sha256'][name + '-' + path.name] = hashlib.sha256(path.read_bytes()).hexdigest()
...
async def capture(*args, **kwargs):
    ...
    observed.append({
        'response': raw,
        'prompt_sha256': hashlib.sha256(args[0].encode()).hexdigest(),
        'system_sha256': hashlib.sha256(args[1].encode()).hexdigest(),
    })
    return result
...
async def main():
    for name in cases:
        ...
        try:
            result = await asyncio.wait_for(service.analyze_retained_source(old['question'], source), 180)
        except Exception as e:
            result = {'status': 'exception', 'error_type': type(e).__name__}
        record = {
            'case': name,
            'result': result,
            'observed': list(observed),
            'seconds': time.monotonic() - start,
            'coverage': {k: source[k] for k in ['page_count', 'readable_pages', 'missing_pages', 'characters', 'input_sha256']},
        }
