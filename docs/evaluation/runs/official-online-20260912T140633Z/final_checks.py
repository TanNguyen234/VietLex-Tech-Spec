...
async def main():
    ...
    result = {
        ...
        'reason': 'Explicit retry of recorded source_transport_error; first attempt retained unchanged.',
    }
    try:
        result['source'] = await read_source(case['reference_url'], use_ocr=True, page_limit=2)
    except SourceReadError as error:
        result['error'] = error.kind
    (out / 'maritime-retry.json').write_text(
