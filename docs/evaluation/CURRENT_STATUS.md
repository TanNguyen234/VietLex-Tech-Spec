...
P3 experiment `structural-e5-384-cap8-20260812` changed only the per-document candidate cap from `4` to `8`. It improved fused all-required coverage from `36/40` to `38/40` and required-evidence survival from `49/53` to `51/53`, with zero technical errors; mean latency changed from `4.478110s` to `3.636966s`. Final all-required coverage remained `0.725` because the configured reranker/final selector still removed verified evidence. Cap `8` is retained for the next candidate-input baseline, but production cutover remains unauthorized.

The exact-input reranker A/B is retained at `docs/evaluation/comparisons/structural-reranker-cap8-20260813/`; comparison SHA-256 is `5144f2e965d4bee97f629a7b4fea6f756acccfb91f904431d0ca93361fb86a49`. All 40 per-case query/document input hashes matched. Pinecone `bge-reranker-v2-m3` improved final all-required coverage from `0.725` to `0.85`, preserved document/article/clause reranker-input recall without loss, and reduced mean total latency from `4.099180s` to `1.712522s`; both runs had zero technical errors. BGE is selected only for the next isolated P3 experiment. Production cutover remains unauthorized because candidate input Article Recall is `0.9333`, Clause Recall is `0.8571`, and final all-required coverage is below `0.95`. The earlier `structural-e5-384-cap8-hashed-qdrant-20260813` run is superseded because it predated observed-provider persistence and is not part of the comparison.

The next P3 implementation now supports opt-in, bounded same-document structural-neighbor expansion (same-article sibling clauses plus numeric `Điều N±1`) before the candidate cap. It is payload-only, deduplicated, revision-checked, strictly read-bounded, and fail-closed; the production factory remains unchanged and the default is off. Two preflight artifacts (`structural-e5-384-cap8-neighbors-pinecone-20260814` and `...-v2-20260814`) record missing backend enablement and a missing Pinecone compatibility alias. The alias was fixed without changing credentials. The v3 run executed all 40 cases but is `BLOCKED_TECHNICAL`: both Qdrant lanes returned `UnexpectedResponse` for every case because the configured cluster endpoint returns HTTP/gRPC `404`, before neighbor expansion or reranking. These runs are not quality evidence. The user subsequently authorized pursuing an isolated replacement knowledge base; the production route and v1 corpus remain unchanged until that replacement passes the declared gates.

The replacement-model reference at `docs/evaluation/index-pilots/pinecone-llama-reference-20260814/reference-probe.json` evaluates Pinecone-hosted `llama-text-embed-v2` at 1024 dimensions on the exact 40 verified queries against 2,573 real structural rows (1,748 relevant rows plus 825 corpus negatives). It records Recall@1 `0.90`, Recall@3 `0.975`, and MRR `0.9388157895`; 39/40 cases place a relevant row in the top 3, while `case_323` first appears at rank 19. This inference-only result made 28 embedding calls and wrote no vectors. It supports an isolated Pinecone structural canary, but does not satisfy the P3 production gate or authorize cutover by itself.
...
The isolated collection `vietlex-legal-rag-v2-pilot-384` was verified green with 134,334 structural records by its immutable upload/finalize/verify chain. On 2026-08-14 the configured endpoint returned `404`, so current reachability and record health are not demonstrated. Pinecone v1 remains the production backend and `get_legal_retriever()` is unchanged. The Qdrant path is still opt-in and is not wired into production. No Pinecone data, local corpus/index, production route, generation, Ragas, guardrail, or deployment state was changed.
...
The follow-up mxbai attempt is retained at `docs/evaluation/index-pilots/structural-recall-hardening-mxbai-20260811/`. Plan file SHA-256 is `e09f587661862243be1f275e1f0ba3c933b460be413912a0bfd4f4cd5bc3ca69`; internal plan SHA-256 is `47d1ab5b5faaaf755fd2b703ff0a31f89457096dedc2a5443ef02788daaf76fa`; adopted-empty receipt SHA-256 is `f7591c0667cc927e959f7490eacbd0f66ae53c80ee513ce8496af958154e5b4c`. Raw upsert proved mxbai is not allowed on free tier, so its probe stopped before commit; probe file SHA-256 is `a99d66927ef104afba95620692ad6d3b80d597fa3eec485072c05bd277b4e00e` and remote count returned to zero. Exact one-record probes established that the free multilingual option is `intfloat/multilingual-e5-small` at 384d; no tested free-tier candidate above 384d was allowed. A new 384d collection is therefore required; the empty 1024d collection is not deleted or reused with an incompatible schema.
