...
| Workspace | 7 mục Tổng quan/Nguồn/Tài liệu/Phân tích/Rà soát/Báo cáo/Hoạt động | `tests/test_workspace_routes.py`, `tests/product_forms.test.cjs` | NOT RUN | Cần JavaScript để thu gọn các panel; lựa chọn ID nguồn giữ qua tải lại trong sessionStorage của tab; giới hạn retention hiện có |
...
| Báo cáo editable / version / export | Có preview Markdown an toàn, citation về snapshot, editor, lưu bản mới, MD/DOCX, in/lưu PDF qua browser | `tests/test_report_deliverables.py`, `tests/test_research_report_routes.py` | NOT RUN | DOCX cơ bản, URL dạng text; PDF không có renderer server; 50 analysis gần nhất; bản sửa luôn unverified; chưa mọi citation link tới đúng khoản |
| Official portal discovery | WebForms GET/POST, thêm đề xuất từ khóa có chỉnh sửa; không tạo số hiệu đoán | `tests/services/test_official_query_planner.py`, `tests/services/test_deep_research.py` | Local live: 13/13 neo; production workflow mới NOT RUN | Mặc định vẫn một cổng; title hit không chứng minh trả lời đủ |
| Đọc nguồn ngoài corpus | PDF chính thức, OCR, đọc tiếp, ghim exact quote với hash và số trang | `tests/services/test_official_document_reader.py`, `tests/test_trusted_source_routes.py` | Local live: 12/13 lần đầu; 13/13 sau retry; API ghim/phân tích thành công | 20 MB, 200 trang/tệp, 5 trang/lượt; benchmark đọc 2 trang; OCR có lỗi quan sát được; hiệu lực unverified |
| Thư viện bản đọc / đối chiếu | Mở lại không gọi AI, nhóm URL + hash PDF, đếm trang có text, lọc nguồn và ghi chú có version CAS | `tests/test_source_library.py`, `tests/test_evidence_review.py` | Local MongoDB/API/Chrome thật đã chạy; xem report mới | 50 analysis gần nhất; 10 sự kiện đối chiếu; đếm trang không chứng minh đúng OCR/đúng luật; không sửa quote gốc |
