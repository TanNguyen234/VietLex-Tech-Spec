...
@pytest.mark.asyncio
async def test_upload_uses_batch_embedding_provider_when_available(tmp_path) -> None:
    from app.ingestion.structural_index import StructuralRecord
    from app.ingestion.vertex_qdrant_migration import (
        MigrationCheckpoint,
        VertexQdrantContract,
    ...
    class Provider:
        ...
        async def embed_documents(self, documents, *, output_dimensionality):
            self.batch_calls.append((documents, output_dimensionality))
            return [SimpleNamespace(values=(0.0,) * output_dimensionality) for _ in documents]

        async def embed_document(self, **_kwargs):
            raise AssertionError("single-document fallback should not be used")
    ...
    assert report == {"attempted": 2, "uploaded": 2, "skipped": 0}
    assert len(provider.batch_calls) == 1
    assert len(provider.batch_calls[0][0]) == 2
